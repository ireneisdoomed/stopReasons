...
def prepare_data(df):
    ...
    data_inputs, data_masks = preprocessing_for_bert(
        df[df["why_stopped"].notnull()].why_stopped
    )
...
def main(input_file, model_path: str, output_file: str) -> None:
    ...
    for ind, row in stopped.iterrows():
        ...
        class_indices = sorted(
            [j for j in range(len(probs[i])) if probs[i][j] >= 0.01], reverse=True
        )
        ...
        writer1.writerow(
            [row["why_stopped"].replace("\r~", ""), subclasses_all, superclasses_all]
        )
